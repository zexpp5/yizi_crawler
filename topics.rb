...
for i in (1  .. 30) do
  ...
  page.css('.fn-clear').each{ |item|
    item.css('.tree-title1,.tree-title2,.tree-title3').each{ |item1|
      out = ""
      ...
      item1.css(".item").each{|item2|
        if out.index("-1-") == 0 then parent = item2["data-nselect"]; level = 1 end
        if out.index("-2-") == 0 then parent = item2["data-nselect"]; level = 2 end
        if out.index("-3-") == 0 then level = 3 end
